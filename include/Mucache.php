...
class mucache{
	...
	private $persistent = true; //是否长连接
	private $prefix = ''; //Key前缀

	public function __construct( $arrServers){ //构造函数	
	...
	private function connect(){
		if ( ! $this->connected){
			...
			foreach ((array)$this->arrServers as $key => $arrSever){//创建多组
				...
				$this->persistent && count($this->arrOmem[$key]->getServerList()) ? '' : $this->arrOmem[$key]->addServers( $arrSever);
			}
		}
	}
